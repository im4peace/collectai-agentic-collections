"""Shared fixtures for `bt/redteam/` tests: one embedded PostgreSQL server
per test session and a migrated schema. Mirrors `bt/api/conftest.py`'s
embedded-postgres pattern rather than importing it, so `bt/redteam/` stays
independently runnable and does not risk perturbing another story's own
conftest (that file's own docstring states the same convention).
"""
...
def pg_server(tmp_path_factory: pytest.TempPathFactory) -> Iterator[ep.PostgresServer]:
    pgdata = tmp_path_factory.mktemp("collectai_redteam_pgdata", numbered=True)

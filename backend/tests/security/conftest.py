"""Shared fixtures for `bt/security/` tests: one embedded PostgreSQL server
per test session and a migrated schema. Mirrors `bt/api/conftest.py`'s
embedded-postgres pattern rather than importing it, matching `bt/redteam
/conftest.py`'s own copy of the same convention.
"""
...
def pg_server(tmp_path_factory: pytest.TempPathFactory) -> Iterator[ep.PostgresServer]:
    pgdata = tmp_path_factory.mktemp("collectai_security_pgdata", numbered=True)
